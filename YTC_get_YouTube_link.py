...
# --- Get live YouTube URL (from channel ID or handle) ---
def get_live_watch_url(channel_id):
    url = f"https://www.youtube.com/{channel_id}/live"  # ✅ updated format
    ydl_opts = {
    ...
    try:
        ...
        logger.warning(f"Failed to extract info for {channel_id}")
        return None
...
def main():
    ...
    for channel_id, metadata in channel_metadata.items():  # ✅ using channel_id (can be @handle or UCID)
